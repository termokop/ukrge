...
if ($jwt) {
    ...
    try {
        ...
        $oldProgres_course_1 = $user->get_user_progress($data->email);
        $oldProgres_course_1 = json_decode($oldProgres_course_1);
        $lesson = (int)$data->lesson;
        $current_lesson = "L_" . strval($lesson);
        $score = strval($data->score);
        if($oldProgres_course_1->$current_lesson == null) return;
        if((int)$oldProgres_course_1->$current_lesson >= (int)$score) $resultObj = $oldProgres_course_1;
        else {
            $oldProgres_course_1->$current_lesson = $score;
            $next_lesson = "L_" . strval($lesson+1);
            if(!isset($oldProgres_course_1->$next_lesson) && $oldProgres_course_1->$current_lesson>=6) {
                $oldProgres_course_1->$next_lesson = "0";
            }
        }
        
        
        $user->email = $data->email;
        $user->course_1 = json_encode($oldProgres_course_1);
